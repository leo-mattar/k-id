...
// --- GLOBAL - SIMPLE FADE
function simpleFade() {
  gsap.set("[simple-fade]", { opacity: 0 });

  let elements = $("[simple-fade]");
  elements.each(function (index, element) {
    ScrollTrigger.create({
      trigger: element,
      start: "-50% bottom",
...
function headerScroll() {
  ...
  headers.forEach(header => {
    // Skip headers with the exclusion attribute
    if (header.hasAttribute("data-exclude-scroll")) return;
...
//   const dropdownWraps = document.querySelectorAll(".c-dropdown-wrap");

//   dropdownWraps.forEach(wrap => {
//     const buttonEl = wrap.querySelector(".c-btn");
//     const dropdownEl = wrap.querySelector(".c-dropdown");
//     const buttonArrow = wrap.querySelector(".c-icon.nav-arrow");
...
//       pointerEvents: "none", // Prevent flickering when hidden
//     });
...
//       height: "auto",
//       onStart: () => {
//         dropdownEl.style.pointerEvents = "auto"; // Enable interactions when open
//       },
//       onReverseComplete: () => {
//         dropdownEl.style.pointerEvents = "none"; // Disable interactions when closed
//       },
//     });
...
//         const otherButton = otherWrap.querySelector(".c-btn");
//         if (otherButton !== this && otherButton.classList.contains("is-open")) {
//           otherButton.classList.remove("is-open");
//           const otherTimeline = otherWrap.gsapTimeline;
//           otherTimeline && otherTimeline.reverse();
//         }
//       });
...
//     // Add an event listener to close dropdown and sub-dropdowns when clicking on .c-sub-dd-link
//     const subDdLinks = wrap.querySelectorAll(".c-sub-dd-link");
//     subDdLinks.forEach(link => {
//       link.addEventListener("click", function () {
//         // Close the main dropdown if it's open
...
//         const openSubDropdowns = document.querySelectorAll(
//           ".c-sub-dd .c-sub-dd-toggle.is-open"
//         );
//         openSubDropdowns.forEach(subToggle => {
//           subToggle.classList.remove("is-open");
//           const subTimeline = subToggle.closest(".c-sub-dd").gsapTimeline;
//           if (subTimeline) {
//             subTimeline.reverse();
//           }
//         });
...
//     wrap.gsapTimeline = tl;
//   });

//   // Global mouseup to close dropdowns, except when clicking inside .c-btn or .c-sub-dd
//   document.addEventListener("mouseup", function (e) {
//     if (!e.target.closest(".c-btn") && !e.target.closest(".c-sub-dd")) {
//       document.querySelectorAll(".c-btn.is-open").forEach(btn => btn.click());
//     }
//   });
// }
  ...
  dropdownWraps.forEach(wrap => {
    ...
    wrap.addEventListener("mouseenter", () => {
      dropdownWraps.forEach(otherWrap => {
        if (otherWrap !== wrap) {
          const otherButton = otherWrap.querySelector(".c-btn");
          if (otherButton.classList.contains("is-open")) {
            otherButton.classList.remove("is-open");
            const otherTimeline = otherWrap.gsapTimeline;
            otherTimeline && otherTimeline.reverse();
          }
...
// --- HEADER SUB DROPDOWN
function subDropdowns() {
  const subDropdowns = document.querySelectorAll(".c-sub-dd");
  if (!subDropdowns.length) return;

  subDropdowns.forEach(sub => {
    const toggle = sub.querySelector(".c-sub-dd-toggle");
    const list = sub.querySelector(".c-sub-dd-list");
    ...
    toggle.addEventListener("click", function (e) {
      ...
      subDropdowns.forEach(otherSub => {
        if (otherSub !== sub) {
          const otherToggle = otherSub.querySelector(".c-sub-dd-toggle");
          if (otherToggle.classList.contains("is-open")) {
            otherToggle.classList.remove("is-open");
            if (otherSub.gsapTimeline) {
              otherSub.gsapTimeline.reverse();
            }
          }
        }
      });
...
function headerMobile() {
  ...
  // Close header when clicking on a .c-sub-dd-link
  $(".c-sub-dd-link").on("click", function () {
    if (headerTrigger.hasClass("is-open")) {
      headerTrigger.removeClass("is-open");
      headerTriggerText.text("menu");
      lenis.start();
      tl.reverse();
    }
...
// --- AGE FAQ CARD FLIP ANIMATION
function ageFlipCard() {
  const cards = document.querySelectorAll(".c-age-item.faq");
